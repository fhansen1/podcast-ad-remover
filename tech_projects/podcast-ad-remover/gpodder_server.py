...
def get_feed(podcast_name):
    ...
    # Check session first, then basic auth
    ...
    # Use FEEDS dictionary (loaded from OPML)
    logger.info(f"Looking for podcast: {podcast_name}")
    logger.info(f"FEEDS type: {type(FEEDS)}, len: {len(FEEDS)}")
    logger.info(f"'det_store_bilded' in FEEDS: {"det_store_bilded" in FEEDS}")
    original_rss = FEEDS.get(podcast_name)
    logger.info(f"original_rss: {original_rss}")
    if not original_rss:
        # Try exact match
        for k in FEEDS.keys():
            if 'det' in k and 'store' in k:
                logger.info(f"Found similar key: {k}")
